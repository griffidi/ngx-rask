...
@Injectable()
export class ObservableState<T extends Record<string, unknown>> implements OnDestroy {
  ...
  public initialize(state: T, inputState$?: Observable<Partial<T>>): void {
    ...
    inputState$?.pipe(takeUntil(this.destroy$$)).subscribe((res: Partial<T>) => this.patch(res));
  }
  ...
  public connect(object: Partial<{ [P in keyof T]: Observable<T[P]> }>): void {
    Object.keys(object).forEach((key: keyof Partial<T>) => {
      object[key]?.pipe(takeUntil(this.destroy$$)).subscribe((v: Partial<T>[keyof Partial<T>]) => {
  ...
  public onlySelectWhen(keys: (keyof T)[]): Observable<T> {
    return this.state$$.syncState.pipe(
      filterAndCastToT<T>(),
      distinctUntilChanged((previous: T, current: T) =>
        keys.every((key: keyof T) => current[key as keyof T] === previous[key as keyof T])
      ),
      takeUntil(this.destroy$$)
    );
  }
  ...
  public pick<P>(keys: (keyof T)[]): Partial<{ [P in keyof T]: Observable<T[P]> }> {
    ...
    keys.forEach((key: keyof T) => {
      returnObj[key] = this.onlySelectWhen([key]).pipe(map((state: T) => state[key]));
    });
    return returnObj;
  }
