 ...
 * @publicApi
 */
@Directive({ selector: '[rkUnpatch]', standalone: true })
export class RkUnpatch {
  #events = signal<string[]>(zonePatchedEvents);
  ...
  constructor() {
    effect(() => {
      this.#reapplyUnPatchedEventListeners(this.#events());
    });
  }

  #reapplyUnPatchedEventListeners(events: string[]): void {
