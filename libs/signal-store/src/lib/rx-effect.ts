...
import { isObservable, Observable, of, Subject, Subscription, takeUntil } from 'rxjs';
...
export function rxEffect<Input>(
  ...
  return input => {
    ...
    return input$.pipe(takeUntil(destroy$)).subscribe(value => source$.next(value));
  };
}

function fromRxInput<Input>(input: Input | Observable<Input> | Signal<Input>): Observable<Input> {
  if (isObservable(input)) {
    return input;
  }

  return typeof input === 'function' && isSignal(input) ? toObservable(input) : of(input);
}
