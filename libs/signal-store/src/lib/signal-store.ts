...
import {
  ...
  type SignalStoreFeatureFactory,
  type SignalStoreUpdate,
  type SignalStoreUpdateFn,
  type StaticState,
} from './models';
...
> = (
  ...
  } & SignalStoreUpdate<StaticState<F1['state'] & F2['state'] & F3['state'] & F4['state']>>
) => F5;
...
> = (
  input: {
    ...
    computed: F1['computed'] & F2['computed'] & F3['computed'] & F4['computed'] & F5['computed'];
    updaters: F1['updaters'] & F2['updaters'] & F3['updaters'] & F4['updaters'] & F5['updaters'];
    effects: F1['effects'] & F2['effects'] & F3['effects'] & F4['effects'] & F5['effects'];
  } & SignalStoreUpdate<
    StaticState<F1['state'] & F2['state'] & F3['state'] & F4['state'] & F5['state']>
  >
) => F6;
...
type F2Result<F1 extends SignalStoreFeature, F2 extends SignalStoreFeature> = FeatureResult<F1> &
  FeatureResult<F2> &
  SignalStoreUpdate<StaticState<F1['state'] & F2['state']>>;
...
> = FeatureResult<F1> &
  ...
  FeatureResult<F6> &
  SignalStoreUpdate<
    StaticState<F1['state'] & F2['state'] & F3['state'] & F4['state'] & F5['state'] & F6['state']>
  >;

export function signalStore<F1 extends SignalStoreFeature>(f1: F1Factory<F1>): Type<F1Result<F1>>;
export function signalStore<F1 extends SignalStoreFeature, F2 extends SignalStoreFeature>(
  f1: F1Factory<F1>,
  f2: F2Factory<F1, F2>
): Type<F2Result<F1, F2>>;
export function signalStore<
  ...
  F2 extends SignalStoreFeature,
  F3 extends SignalStoreFeature
>(f1: F1Factory<F1>, f2: F2Factory<F1, F2>, f3: F3Factory<F1, F2, F3>): Type<F3Result<F1, F2, F3>>;
export function signalStore<
  F1 extends SignalStoreFeature,
  F2 extends SignalStoreFeature,
  F3 extends SignalStoreFeature,
...
export function signalStore(
  ...args: [SignalStoreConfig, ...SignalStoreFeatureFactory[]] | SignalStoreFeatureFactory[]
) {
  ...
  for (const featureFactory of featureFactories) {
    ...
    if (ngDevMode) {
      if (!!debugId && !ngTestMode) {
        function decorateUpdatersOrFeatures(
          type: string,
          arr: Record<string, (...args: any[]) => void>
        ) {
    ...
    if (!!debugId && !ngTestMode) {
      ...
      debugWatcher = effect(() => {
        ...
        const o: Record<string, string> = Object.fromEntries(
          diff
            .filter(diffResult => diffResult.path)
            .map(diffResult => [
              diffResult.op.toUpperCase() + ': ' + diffResult.path.join('.'),
              diffResult.val,
            ])
        );
